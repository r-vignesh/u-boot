...
int board_fit_config_name_match(const char *name)
{
	...
	if (!eeprom_read || board_is_am62x_lp_skevm()) {
...
static int __maybe_unused init_daughtercard_det_gpio(char *gpio_name, struct gpio_desc *desc)
{
	int ret;
...
static int __maybe_unused probe_daughtercards(void)
{
